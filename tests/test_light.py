...
valid_ctl = [
    ...
    pytest.param(
        ...
        LightCTLOpcode.LIGHT_CTL_TEMPERATURE_STATUS,
        {
            "present_ctl_temperature": 0x1111,
...
def test_build_valid_lightness(encoded, opcode, data):
    assert LightLightnessMessage.build({"opcode": opcode, "params": data}) == encoded
...
def test_build_valid_lightness_setup(encoded, opcode, data):
    assert LightLightnessSetupMessage.build({"opcode": opcode, "params": data}) == encoded
...
def test_build_valid_ctl(encoded, opcode, data):
    assert LightCTLMessage.build({"opcode": opcode, "params": data}) == encoded
...
def test_build_valid_ctl_setup(encoded, opcode, data):
    assert LightCTLSetupMessage.build({"opcode": opcode, "params": data}) == encoded
