...
from bluetooth_mesh.messages import GenericOnOffMessage, GenericOnOffOpcode
from bluetooth_mesh.messages.generics import TransitionTime
...
def test_build_transition_time(steps, resolution, expected_tt):
    tt = TransitionTime.build({"steps": steps, "resolution": resolution})
...
valid = [
    pytest.param(b"\x82\x01", GenericOnOffOpcode.GENERIC_ONOFF_GET, {}, id="ONOFF_GET"),
    pytest.param(
        b"\x82\x02\x01\x22",
...
def test_build_valid(encoded, opcode, data):
    assert GenericOnOffMessage.build({"opcode": opcode, "params": data}) == encoded
